...
func (iter *NodeIterator[N]) Consume() (tree.Noder, error) {
	if iter.current == nil {
...
func (tn *Node[N]) GetParent() tree.Noder {
	return tn.Parent
}
...
func (tn *Node[N]) Cleanup() []tree.Noder {
...
func (tn *Node[N]) DeleteChild(target tree.Noder) []tree.Noder {
	if target == nil {
		return nil
	}
...
func (tn *Node[N]) GetFirstChild() tree.Noder {
	return tn.FirstChild
}

// AddChild implements the tree.Noder interface.
func (tn *Node[N]) AddChild(target tree.Noder) {
	if target == nil {
		return
	}
...
func (tn *Node[N]) LinkChildren(children []tree.Noder) {
...
func (tn *Node[N]) RemoveNode() []tree.Noder {
	prev := tn.PrevSibling
	next := tn.NextSibling
	parent := tn.Parent
...
func (tn *Node[N]) Iterator() common.Iterater[tree.Noder] {
...
func (tn *Node[N]) String() string {
	return fmt.Sprintf("Node[N][%v]", tn.Data)
}
